...
export default async (req: Request, res: Response) => {
  ...
  function isCandidatePresent(candidates: any, candidateName: string) {
    for (let i = 0; i < candidates.length; i++) {
      if (candidates[i][0] === candidateName) {
  ...
  if (!isCandidatePresent(candidates, req.body.candidate))
    return res.status(400).send({ a: "no such candidate", candidates });
